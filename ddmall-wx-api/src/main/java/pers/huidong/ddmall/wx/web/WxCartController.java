...
public class WxCartController {
    ...
    public Object index(@LoginUser Integer userId) {
        ...
        List<DdmallCart> list = cartService.queryByUid(userId);
        List<DdmallCart> cartList = new ArrayList<>();
        // TODO
        // 如果系统检查商品已删除或已下架，则系统自动删除。
        // 更好的效果应该是告知用户商品失效，允许用户点击按钮来清除失效商品。
        for (DdmallCart cart : list) {
            DdmallGoods goods = goodsService.findById(cart.getGoodsId());
     ...
     */
    @PostMapping("add")
    public Object add(@LoginUser Integer userId, @RequestBody DdmallCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
        ...
        DdmallGoodsProduct product = productService.findById(productId);
        //判断购物车中是否存在此规格商品
        DdmallCart existCart = cartService.queryExist(goodsId, productId, userId);
        if (existCart == null) {
            //取得规格的信息,判断规格库存
     ...
     */
    @PostMapping("fastadd")
    public Object fastadd(@LoginUser Integer userId, @RequestBody DdmallCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
     ...
     */
    @PostMapping("update")
    public Object update(@LoginUser Integer userId, @RequestBody DdmallCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        Integer productId = cart.getProductId();
        ...
        //判断是否存在该订单
        // 如果不存在，直接返回错误
        DdmallCart existCart = cartService.findById(userId, id);
        if (existCart == null) {
    ...
    public Object goodscount(@LoginUser Integer userId) {
        ...
        int goodsCount = 0;
        List<DdmallCart> cartList = cartService.queryByUid(userId);
        for (DdmallCart cart : cartList) {
            goodsCount += cart.getNumber();
        }
    ...
    public Object checkout(@LoginUser Integer userId, Integer cartId, Integer addressId, Integer couponId, Integer userCouponId, Integer grouponRulesId) {
        ...
        // 收货地址
        DdmallAddress checkedAddress = null;
        if (addressId == null || addressId.equals(0)) {
            checkedAddress = addressService.findDefault(userId);
            // 如果仍然没有地址，则是没有收货地址
            // 返回一个空的地址id=0，这样前端则会提醒添加地址
            if (checkedAddress == null) {
                checkedAddress = new DdmallAddress();
        ...
        DdmallGrouponRules grouponRules = grouponRulesService.findById(grouponRulesId);
        if (grouponRules != null) {
            grouponPrice = grouponRules.getDiscount();
        }

        // 商品价格
        List<DdmallCart> checkedGoodsList = null;
        if (cartId == null || cartId.equals(0)) {
            checkedGoodsList = cartService.queryByUidAndChecked(userId);
        } else {
            DdmallCart cart = cartService.findById(userId, cartId);
            if (cart == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        for (DdmallCart cart : checkedGoodsList) {
        ...
        List<DdmallCouponUser> couponUserList = couponUserService.queryAll(userId);
        for(DdmallCouponUser couponUser : couponUserList){
            DdmallCoupon coupon = couponVerifyService.checkCoupon(userId, couponUser.getCouponId(), couponUser.getId(), checkedGoodsPrice, checkedGoodsList);
            if(coupon == null){
                continue;
            }
        ...
        if (couponId == null || couponId.equals(-1)){
            ...
            DdmallCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedGoodsPrice, checkedGoodsList);
