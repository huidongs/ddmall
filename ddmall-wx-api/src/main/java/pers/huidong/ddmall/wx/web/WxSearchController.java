...
import pers.huidong.ddmall.db.service.DdmallKeywordService;
...
public class WxSearchController {
    ...
    @Autowired
    private DdmallKeywordService keywordsService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        //取出输入框默认的关键词
        DdmallKeyword defaultKeyword = keywordsService.queryDefault();
        //取出热闹关键词
        List<DdmallKeyword> hotKeywordList = keywordsService.queryHots();

        List<DdmallSearchHistory> historyList = null;
        if (userId != null) {
            //取出用户历史关键字
    ...
    public Object helper(@NotEmpty String keyword,
        ...
        List<DdmallKeyword> keywordsList = keywordsService.queryByKeyword(keyword, page, limit);
        String[] keys = new String[keywordsList.size()];
        int index = 0;
        for (DdmallKeyword key : keywordsList) {
